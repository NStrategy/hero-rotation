    ...
    elseif Settings.Outlaw.RolltheBonesLogic == "True Bearing" then
      ...
      if not Player:HasTier(31, 4) then
        if RtB_Buffs() == 0 or (RtB_Buffs() == 1 and not Player:BuffUp(S.TrueBearing)) then
          Cache.APLVar.RtB_Reroll = true
        elseif RtB_Buffs() == 1 and not Player:BuffUp(S.Broadside) then
          Cache.APLVar.RtB_Reroll = true
        end
      end
...
local function StealthCDs ()
  ...
  if S.Vanish:IsCastable() and (Target:FilteredTimeToDie(">", 10) or HL.BossFilteredFightRemains(">", 10)) and Vanish_DPS_Condition() and S.HiddenOpportunity:IsAvailable() and not S.Crackshot:IsAvailable() and not Player:BuffUp(S.Audacity)
  ...
  if S.Vanish:IsCastable() and S.BetweentheEyes:IsReady() and (Target:FilteredTimeToDie(">", 10) or HL.BossFilteredFightRemains(">", 10)) and Vanish_DPS_Condition() and (not S.HiddenOpportunity:IsAvailable() or S.Crackshot:IsAvailable()) and Finish_Condition() then
  ...
  if S.ShadowDance:IsAvailable() and S.BetweentheEyes:IsReady() and (Target:FilteredTimeToDie(">", 10) or HL.BossFilteredFightRemains(">", 10)) and S.ShadowDance:IsCastable() and S.Crackshot:IsAvailable() and Finish_Condition() and S.Vanish:CooldownRemains() >= (Rogue.CPMaxSpend() * (1 + (Player:BuffUp(S.TrueBearing) and 0.5 or 0))) then
  ...
  if S.ShadowDance:IsAvailable() and (Target:FilteredTimeToDie(">", 10) or HL.BossFilteredFightRemains(">", 10)) and S.ShadowDance:IsCastable() and not S.KeepItRolling:IsAvailable() and Shadow_Dance_Condition() and Player:BuffUp(S.SliceandDice)
  ...
  if S.ShadowDance:IsAvailable() and (Target:FilteredTimeToDie(">", 10) or HL.BossFilteredFightRemains(">", 10)) and S.ShadowDance:IsCastable() and S.KeepItRolling:IsAvailable() and Shadow_Dance_Condition()
...
local function CDs ()
  ...
  -- # Maintain Blade Flurry on 2+ targets, and on single target with Underhanded - Following Rogue Discord FAQ - Only maintain BF in single target when Adrenaline Rush is active.
  ...
  if S.BladeFlurry:IsReady() then
    if (EnemiesBFCount >= 2 or (S.UnderhandedUpperhand:IsAvailable() and Player:BuffUp(S.AdrenalineRush) and EnemiesBFCount == 1) and not Player:StealthUp(true, false))
      ...
      if Settings.Outlaw.GCDasOffGCD.BladeFlurry then
        ...
        if HR.Cast(S.BladeFlurry) then return "Cast Blade Flurry (ST, 2+Targets)" end
      end
    end
