...
local function Finish(ReturnSpellOnly)
  ...
  -- actions.finish+=/between_the_eyes,if=talent.crackshot&(buff.ruthless_precision.up|buff.between_the_eyes.remains<4|!talent.keep_it_rolling|!talent.mean_streak)
	if S.BetweentheEyes:IsCastable() and Target:IsSpellInRange(S.BetweentheEyes) and S.Crackshot:IsAvailable() and (Player:BuffUp(S.RuthlessPrecision) or Player:BuffRemains(S.BetweentheEyes) < 4 or not S.KeepItRolling:IsAvailable() or not S.MeanStreak:IsAvailable()) then
    if ReturnSpellOnly then
      return S.BetweentheEyes
    else
      if CastPooling(S.BetweentheEyes) then return "Cast Between the Eyes" end
...
local function CDs ()
  ...
  if S.KeepItRolling:IsCastable() and Cache.APLVar.RtB_Buffs.Total >= 4 and (Cache.APLVar.RtB_Buffs.MinRemains < 3 or Player:BuffUp(S.Broadside) or Player:BuffUp(S.TrueBearing)) then
  ...
  if S.KeepItRolling:IsCastable() and Cache.APLVar.RtB_Buffs.Total >= 3 and (num(Player:BuffUp(S.Broadside)) + num(Player:BuffUp(S.RuthlessPrecision)) + num(Player:BuffUp(S.TrueBearing)) >= 2) and (Cache.APLVar.RtB_Buffs.MinRemains < 3 or (num(Player:BuffUp(S.Broadside)) + num(Player:BuffUp(S.RuthlessPrecision)) + num(Player:BuffUp(S.TrueBearing) == 3))) then
    if Cast(S.KeepItRolling, Settings.Outlaw.GCDasOffGCD.KeepItRolling) then
      return "Cast Keep it Rolling"
    end
  end
  ...
  if not Player:StealthUp(true, true) and S.Crackshot:IsAvailable() and S.UnderhandedUpperhand:IsAvailable() and S.Subterfuge:IsAvailable() and Player:BuffStack(S.EscalatingBlade) < 4 and Player:BuffUp(S.AdrenalineRush) and Finish_Condition() then
    ...
    if S.Vanish:IsCastable() and not S.KillingSpree:IsAvailable() and not S.BetweentheEyes:IsCastable() and Player:BuffRemains(S.RuthlessPrecision) > 4 and (S.KeepItRolling:CooldownRemains() > 150 and Cache.APLVar.RtB_Buffs.Normal > 0 or not S.Supercharger:IsAvailable()) then
    ...
    if S.Vanish:IsCastable() and Player:BuffRemains(S.AdrenalineRush) < 3 and S.AdrenalineRush:CooldownRemains() > 10 then
    ...
    if S.Vanish:IsCastable() and not S.KillingSpree:IsAvailable() and ChargedComboPoints > 0 then
    ...
    if S.Vanish:IsCastable() and S.KillingSpree:CooldownRemains() > 15 then
      -- if HR.Cast(S.Vanish, Settings.CommonsOGCD.OffGCDasOffGCD.Vanish) then return "Cast Vanish (HO)" end
      ShouldReturn = SpellQueueMacro(S.Vanish)
    ...
    if S.Vanish:IsCastable() and S.Vanish:FullRechargeTime() < 15 then
    ...
    -- fight remains condition of Vanish deleted cause it shit with HR lol.
...
local function APL ()
  ...
  if not Player:AffectingCombat() and S.Vanish:TimeSinceLastCast() > 1 then
    ...
    if not Player:AffectingCombat() then
      ...
      if S.RolltheBones:IsReady() and (Cache.APLVar.RtB_Buffs.Total == 0 or RtB_Reroll() or (LongestRtBRemains() <= 7.5 and DungeonSlice and EnemiesBFCount == 0)) then
