  ...
  if HR.CDsON() then
    ...
    if Settings.Commons.Enabled.Trinkets then
      ...
      if I.AshesoftheEmbersoul:IsEquippedAndReady() then
        if ((((S.ColdBlood:IsCastable() and ComboPoints >= 5) or Player:BuffUp(S.ColdBlood)) and Player:BuffStack(S.DanseMacabreBuff) >= 3) or (not S.DanseMacabre:IsAvailable() and Player:BuffUp(S.ShadowDanceBuff) or Player:BuffStack(S.DanseMacabreBuff) >= 3) and not S.ColdBlood:IsAvailable()) or HL.BossFilteredFightRemains("<", 10) then
