...
local function Finish (ReturnSpellOnly, ForceStealth)
  ...
  if S.SecretTechnique:IsCastable() and (Secret or ForceStealth) then
      ...
      if CastPooling(S.SecretTechnique, nil, not TargetInMeleeRange) then
  ...
  if S.Rupture:IsCastable() and (not Player:BuffUp(S.ShadowDanceBuff) and not SkipRupture and not Skip_Rupture_NPC(Target)) then
    if (not Target:DebuffUp(S.Rupture) or Target:DebuffRefreshable(S.Rupture, RuptureThreshold)) and (Target:FilteredTimeToDie(">", 6, -Target:DebuffRemains(S.Rupture)) or Target:TimeToDieIsNotValid()) then
      if ReturnSpellOnly then
        ...
        if CastPooling(S.Rupture, nil, not TargetInMeleeRange) then
  ...
  if S.Rupture:IsCastable() and not Player:BuffUp(S.ShadowDanceBuff) and not SkipRupture then
  ...
  if S.BlackPowder:IsCastable() then
    if not PriorityRotation and Maintenance and MeleeEnemies10yCount >= 2 and not Player:BuffUp(S.FlawlessFormBuff) and not Player:BuffUp(S.DarkestNightBuff) then
      if ReturnSpellOnly then
  ...
  if S.CoupDeGrace:IsCastable() and Target:DebuffUp(S.FazedDebuff) then
    if ReturnSpellOnly then
      ...
      if CastPooling(S.CoupDeGrace, nil, not TargetInMeleeRange) then
  ...
  if S.Eviscerate:IsCastable() then
    if ReturnSpellOnly then
      ...
      if CastPooling(S.Eviscerate, nil, not TargetInMeleeRange) then
...
local function Build (ReturnSpellOnly, ForceStealth)
  ...
  if S.ShurikenStorm:IsCastable() and HR.AoEON() and S.DeathStalkersMark:IsAvailable() and not Player:BuffUp(S.PremeditationBuff) and MeleeEnemies10yCount >= (2 + 3 * num(Player:BuffUp(S.ShadowDanceBuff))) or Player:BuffUp(S.ClearTheWitnessesBuff) and not Player:BuffUp(S.SymbolsofDeath) or Player:BuffUp(S.FlawlessFormBuff) and MeleeEnemies10yCount >= 3 and not Stealth then
    if ReturnSpellOnly then
      ...
      if CastPooling(S.ShurikenStorm, nil, not TargetInAoERange) then
  ...
  if S.ShurikenTornado:IsCastable() and S.ShurikenTornado:IsAvailable() then
    if Player:BuffUp(S.LingeringDarknessBuff) or S.DeathStalkersMark:IsAvailable() and S.ShadowBlades:CooldownRemains() >= 32 and MeleeEnemies10yCount >= 2 or S.UnseenBlade:IsAvailable() and Player:BuffUp(S.SymbolsofDeath) and MeleeEnemies10yCount >= 4 then
      if ReturnSpellOnly then
        ...
        if CastPooling(S.ShurikenTornado, Settings.Subtlety.GCDasOffGCD.ShurikenTornado, not TargetInAoERange) then
  ...
  if HR.CDsON() and S.GoremawsBite:IsAvailable() and S.GoremawsBite:IsCastable() then
    if ComboPointsDeficit >= 3 then
      if ReturnSpellOnly then
        ...
        if CastPooling(S.GoremawsBite, nil, not TargetInMeleeRange) then
  ...
  if S.Gloomblade:IsCastable() then
    if ReturnSpellOnly then
      ...
      if CastPooling(S.Gloomblade, nil, not TargetInMeleeRange) then
  ...
  if S.Backstab:IsCastable() then
    if ReturnSpellOnly then
      ...
      if CastPooling(S.Backstab, nil, not TargetInMeleeRange) then
...
local function StealthMacro (StealthSpell)
  ...
  if not Player:BuffUp(S.DarkestNightBuff) and EffectiveComboPoints >= 6 or Player:BuffUp(S.DarkestNightBuff) and ComboPoints == Rogue.CPMaxSpend() then
    MacroAbility = Finish(true, StealthSpell)
  end
...
local function CDs ()
  ...
  if Settings.Commons.Enabled.Potions then
    ...
    if PotionSelected and PotionSelected:IsReady() and Player:BloodlustUp() or HL.BossFilteredFightRemains("<", 30) or Player:BuffUp(S.FlagellationBuff) then
      if Cast(PotionSelected, Settings.CommonsDS.DisplayStyle.Potions) then
        return "Cast Potion";
      end
    end
  end
  ...
  if S.ThistleTea:IsCastable() then
    if Player:BuffRemains(S.ShadowDanceBuff) > 2 and Player:BuffDown(S.ThistleTea) then
      if Cast(S.ThistleTea, Settings.CommonsDS.DisplayStyle.Trinkets) then
        return "Thistle Tea";
      end
    end
  end
  ...
  if S.Berserking:IsReady() and RacialSync then
  ...
  if S.Fireblood:IsReady() and RacialSync and Player:BuffUp(S.ShadowDanceBuff) then
  ...
  if S.AncestralCall:IsReady() and RacialSync then
...
local function Stealth_CDs ()
  if HR.CDsON() then
    ...
    if Settings.Commons.ShowPooling and S.Shadowmeld:IsCastable() and Player:Energy() >= 40 and ComboPointsDeficit >= 3 then
...
local function APL ()
  ...
  SkipRupture = Player:BuffUp(S.ShadowDanceBuff) or Player:BuffDown(S.SliceandDice) or Player:BuffUp(S.DarkestNightBuff) or MeleeEnemies10yCount >= 8 and not S.ReplicatingShadows:IsAvailable() and S.UnseenBlade:IsAvailable()

  -- actions+=/variable,name=maintenance,value=(dot.rupture.ticking|variable.skip_rupture)&buff.slice_and_dice.up
  Maintenance = (Target:DebuffUp(S.Rupture) or SkipRupture or Skip_Rupture_NPC(Target)) and Player:BuffUp(S.SliceandDice)
  ...
  RacialSync = (Player:BuffUp(S.FlagellationBuff) and Player:BuffUp(S.ShadowDanceBuff)) or not S.ShadowBlades:IsAvailable() and Player:BuffUp(S.SymbolsofDeath) or (HL.BossFilteredFightRemains("<", 20) and InRaid)

  -- actions+=/variable,name=shd_cp,value=combo_points<=4|buff.darkest_night.up&combo_points>=7|effective_combo_points>=6&talent.unseen_blade
  ShdCp = ComboPoints <= 4 or Player:BuffUp(S.DarkestNightBuff) and ComboPoints >= 7 or EffectiveComboPoints >= 6 and S.UnseenBlade:IsAvailable()
