...
local function CDs ()
  ...
  if TargetInMeleeRange then
    ...
    if HR.CDsON() and (S.Flagellation:IsCastable() and not DungeonSlice or (DungeonSlice and ((S.ShadowBlades:CooldownRemains() <= 35 and (S.Vanish:IsCastable() or S.ShadowBlades:CooldownRemains() <= 3)) or S.ShadowBlades:TimeSinceLastCast() < 7))) and SnDCondition and ComboPoints >= 5 and Target:FilteredTimeToDie(">", 10) and (S.SymbolsofDeath:CooldownRemains() <= 3 or Player:BuffRemains(S.SymbolsofDeath) > 3)
