...
local function CDs ()
  ...
  if HR.CDsON() then
    ...
    if I.Potion:IsReady() then
       if Player:BloodlustUp() 
         or HL.BossFilteredFightRemains("<=", 30) 
         or (Player:BuffUp(S.SymbolsofDeath) and (Player:BuffUp(S.ShadowBlades) or S.ShadowBlades:CooldownRemains() <= 10)) then
         if HR.Cast(I.Potion, nil, Settings.Commons.PotionDisplayStyle) then return "Use Potion"; end
       end
    end
    -- Racials
